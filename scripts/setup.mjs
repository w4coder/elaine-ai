...
function httpPing(url, timeoutMs = 1500) {
  return new Promise((resolveP) => {
    const req = http.get(url, (res) => { res.resume(); resolveP(res.statusCode < 500); });
    req.on('error', () => resolveP(false));
    req.setTimeout(timeoutMs, () => { req.destroy(); resolveP(false); });
  });
}
async function waitFor(url, label, tries = 30) {
  for (let i = 0; i < tries; i++) {
    if (await httpPing(url)) { ok(`${label} is up`); return true; }
...
async function relocateIfNeeded() {
  ...
  if (!which('git')) {
    die('git is required for the npx install path. Install git and re-run, or clone the repo manually and run `npm run setup`.');
  }
...
async function detectOrInstallOllama() {
  ...
  if (IS_WIN) {
    if (!which('winget')) {
      die('winget not found. Install Ollama manually from https://ollama.com/download then re-run.');
    }
...
async function smokeTestOllama(model) {
  ...
  return new Promise((resolveP) => {
    const req = http.request({
      host: '127.0.0.1', port: 11434, path: '/api/generate',
      method: 'POST',
    ...
    req.on('error', (e) => { warn(`Smoke test failed: ${e.message}`); resolveP(false); });
...
function writeProviderHint(root, provider, model) {
  ...
  writeFileSync(hintPath, JSON.stringify({ provider, model, createdAt: new Date().toISOString() }, null, 2));
...
async function buildAndStart(root) {
  ...
  const stop = () => { child.kill('SIGINT'); process.exit(0); };
...
async function main() {
  ...
  if (provider === 'ollama' && providerReady) {
    ...
    const modelAns = (await ask(`Model to pull (default: ${def}, smaller option: qwen2.5:3b)`)).trim();
